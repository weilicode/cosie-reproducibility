...
if __name__ == "__main__":
    file_path = './Fig6_data'
    adata1_rna = sc.read_h5ad(os.path.join(file_path, 'adata_P15_LUAD_Visium_rna_istar.h5ad'))
    adata2_rna = sc.read_h5ad(os.path.join(file_path, 'adata_P12_LUAD_Visium_rna_istar.h5ad'))
    adata3_adt = sc.read_h5ad(os.path.join(file_path, 'adata_P24_LUAD_Visium_adt_istar.h5ad'))
    adata4_adt = sc.read_h5ad(os.path.join(file_path, 'adata_P11_LUAD_Visium_adt_istar.h5ad'))
    ...
    result_path = './Fig6_data/COSIE_result/'
    s1_embedding = np.load(os.path.join(result_path, 's1_embedding.npy'))
    s2_embedding = np.load(os.path.join(result_path, 's2_embedding.npy'))
    s3_embedding = np.load(os.path.join(result_path, 's3_embedding.npy'))
    s4_embedding = np.load(os.path.join(result_path, 's4_embedding.npy'))
    final_embeddings = {'s1':s1_embedding, 's2':s2_embedding, 's3':s3_embedding, 's4':s4_embedding}
    ...
    sorted_cluster_label_return = cluster_and_visualize_superpixel(final_embeddings, 
                                                     ...
                                                     colormap = color_map, 
                                                     save_path = 'COSIE_lung.jpg',
                                                     dpi = 500,
                                                     figscale = 120)
    for highlight_label in [10, 18, 19, 20, 21]:
        highlight_joint_clusters_all_sections(
            ...
            figscale=220,
            dpi=500,
            save_dir = './highlight/cluster_{}'.format(highlight_label),
            bg_color = [200, 200, 200]
        )
    ...
    data_dir = "./Fig6_data/COSIE_result"
    save_dir = "./Fig6_lung_all_quantitative"
    ...
    def plot_bar(df, x, y, ylabel, filename, ylim):
    ...
    def add_fp_rows(rows, sample, modality, fp_df):
    
        rows.append(
            pd.DataFrame({
                "Sample": sample,
                "Modality": modality,
                "Value": fp_df["FP_mean_over_thresholds"]
            })
        )
    ...
    def plot_fp_boxplot():
        ...
        add_fp_rows(rows, "S1", "Protein", s1_pro_fp)
        add_fp_rows(rows, "S2", "Protein", s2_pro_fp)
        add_fp_rows(rows, "S3", "RNA", s3_rna_fp)
        add_fp_rows(rows, "S4", "Protein", s4_pro_fp)
        add_fp_rows(rows, "S4", "RNA", s4_rna_fp)
        ...
        ax.set_ylabel("False positive score")
        ax.set_xlabel("")
        ax.set_ylim(-0.1, 1)
        ...
        save_fig(
            fig,
            "false_positive_boxplot.png"
        )
    
    
    # load prediction metrics
    s1_pro_8 = load_pkl(
        "correlation_metrics_P15_s1_protein.pkl"
    )
    
    s1_pro_112 = load_pkl(
        "correlation_metrics_P15_s1_protein_112.pkl"
    )
    
    s2_pro_8 = load_pkl(
        "correlation_metrics_P12_s2_protein.pkl"
    )
    
    s2_pro_112 = load_pkl(
        "correlation_metrics_P12_s2_protein_112.pkl"
    )
    
    s3_rna_8 = load_pkl(
        "correlation_metrics_P24_RNA_1000hvg.pkl"
    )
    
    s3_rna_112 = load_pkl(
        "correlation_metrics_P24_RNA_1000hvg_112.pkl"
    )
    
    s4_pro_8 = load_pkl(
        "correlation_metrics_P11_protein.pkl"
    )
    
    s4_pro_112 = load_pkl(
        "correlation_metrics_P11_protein_112.pkl"
    )
    
    s4_rna_8 = load_pkl(
        "correlation_metrics_P11_RNA_1000hvg.pkl"
    )
    
    s4_rna_112 = load_pkl(
        "correlation_metrics_P11_RNA_1000hvg_112.pkl"
    )
    
    
    # load false positive metrics
    s1_pro_fp = pd.read_csv(
        os.path.join(
            data_dir,
            "s1_P15_LUAD_Visium_fp_per_protein.csv"
        )
    )
    
    s2_pro_fp = pd.read_csv(
        os.path.join(
            data_dir,
            "s2_P12_LUAD_Visium_fp_per_protein.csv"
        )
    )
    
    s3_rna_fp = pd.read_csv(
        os.path.join(
            data_dir,
            "s3_P24_LUAD_Visium_fp_per_RNA.csv"
        )
    )
    
    s4_pro_fp = pd.read_csv(
        os.path.join(
            data_dir,
            "s4_P11_LUAD_Visium_fp_per_protein.csv"
        )
    )
    
    s4_rna_fp = pd.read_csv(
        os.path.join(
            data_dir,
            "s4_P11_LUAD_Visium_fp_per_RNA.csv"
        )
    )
    
    
    # ARI / NMI
    lung_res = pd.read_csv(
        os.path.join(
            data_dir,
    ...
    plot_df = (
        lung_res[
            lung_res["annotation_key"] == "annotations_clean"
        ]
        .copy()
        .sort_values("sid")
    )

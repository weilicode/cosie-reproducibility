...
if __name__ == "__main__":
    adata1_rna = sc.read_h5ad('./Fig5_data/adata_PCA-NCI-E44_rna.h5ad')
    adata2_adt = sc.read_h5ad('./Fig5_data/adata_comet_normalized.h5ad')
    print(adata2_adt.var_names)
    ...
    s1_embedding = np.load('./Fig5_data/COSIE_result/s1_embedding.npy')
    s2_embedding = np.load('./Fig5_data/COSIE_result/s2_embedding.npy')
    final_embeddings = {
        's1':s1_embedding,
        's2':s2_embedding,
    }
    ...
    cluster_label = cluster_and_visualize_superpixel(final_embeddings, 
                                                     ...
                                                     vis_basis="spatial",  
                                                     colormap = color_list, 
                                                     save_path='COSIE_clustering_gastric.jpg',
                                                     dpi = 500,
                                                     figscale = 220)
    ...
    adata1_protein_imputed_norm = create_normalized_adata(adata1_protein_imputed_norm)
    adata2_adt_norm = create_normalized_adata(adata2_adt_norm)

    for protein in ['Cytokeratin', 'aSMA', 'Ecadherin']:
        plot_marker_comparison_superpixel(
            ...
            figscale = 220,
            dpi = 500, 
            save_path = './COSIE_prediction/section1_protein/{}.jpg'.format(protein))

    adata2_rna_imputed = sc.read_h5ad('./Fig5_data/COSIE_result/adata2_rna_imputed.h5ad')
    adata2_rna_imputed_norm = create_normalized_adata(adata2_rna_imputed)
    adata1_rna_norm = create_normalized_adata(adata1_rna)
    for gene in ['GKN1', 'ATP4A', 'DES', 'TFF1', 'PGA5', 'CNN1']:
        plot_marker_comparison_superpixel(
            gene,
            adata2_rna_imputed_norm,
            adata1_rna_norm,
            section1_label = 'Section 2 Imputed',
            section2_label = 'Section 1 Observed',
            basis = 'spatial',
            colormap = "turbo",
            figscale = 220,
            dpi = 500,  
            save_path = './COSIE_prediction/section2_gene/{}.jpg'.format(gene))
